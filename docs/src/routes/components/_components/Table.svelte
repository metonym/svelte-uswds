<script>
  import { Grid, Checkbox, Table, TableHead, TableBody, TableRow, TableCell } from "svelte-uswds";
  import Layout from "./Layout.svelte";

  let borderless = false;

  $: props = [`${borderless ? ` borderless` : ""}`].join("");
  $: code = `<script>
  import { Table, TableHead, TableBody, TableRow, TableCell } from 'svelte-uswds';
<\/script>

<Table${props} caption="Table caption">
  <TableHead>
    <TableRow>
...
<Layout componentName="Table" {code}>
  <Table {borderless} caption="Table caption">
    <TableHead>
      <TableRow>
        <TableCell>Document title</TableCell>
        <TableCell>Description</TableCell>
  ...
  <div slot="props">
    <Grid row gutters>
      <Grid col auto>
        <Checkbox bind:checked={borderless} value="borderless">
          <code class="font-mono-xs padding-05 radius-sm">borderless</code>
        </Checkbox>
      </Grid>
    </Grid>
  </div>
